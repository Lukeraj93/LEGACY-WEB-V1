(function initAdminDashboard() {
  ...
  function renderQaAccountStatus() {
    ...
    if (!status) {
      ...
      qaAccountsDetailsNode.innerHTML = `
        <article class="dashboard-note">
          <p>The current QA account details will appear here once the status request completes.</p>
        </article>
      `;
  ...
  function renderCoachReviewQueue(data) {
    ...
    coachReviewRowsNode.innerHTML = rows.length
      ...
      : `
        <tr>
          <td colspan="6">No coach review data has been configured yet.</td>
        </tr>
      `;
  }
                ...
                return `
                  <article class="lead-chip-card">
                    ...
                    <div class="lead-chip-card__meta">
                      <span class="chip chip--accent">${escapeHtml(lead.source || "Unknown source")}</span>
                      <span class="chip">${escapeHtml(ownerName)}</span>
                    </div>
                    <small>${escapeHtml(lead.email || lead.phone || "No direct contact saved")}</small>
                    <p>${escapeHtml(`Follow-up: ${followUpLabel}`)}</p>
                  </article>
                `;
              })
              .join("")
  ...
  function renderFollowUpQueue(data) {
    ...
    overdueFollowUpRowsNode.innerHTML = queue.length
      ...
      : `
        <tr>
          <td colspan="5">No overdue or upcoming follow-ups yet.</td>
        </tr>
      `;
  }
  ...
  function renderCoachTable(data) {
    ...
    coachRowsNode.innerHTML = data.coaches.length
      ...
      : `
        <tr>
          <td colspan="7">No coach records yet.</td>
        </tr>
      `;
  }
  ...
  function renderClientManagement(data) {
    ...
    clientRowsNode.innerHTML = clients.length
      ...
      : `
        <tr>
          <td colspan="4">No client records yet.</td>
        </tr>
      `;
  ...
  function renderActivationCodeControls(data) {
    ...
    adminAuthCodeRowsNode.innerHTML = rows.length
      ...
      : `
        <tr>
          <td colspan="6">No authentication codes generated yet.</td>
        </tr>
      `;
  }
  ...
  function renderLeadActivities(data) {
    ...
    leadActivityRowsNode.innerHTML = data.leadActivities.length
      ...
      : `
        <tr>
          <td colspan="5">No lead activity yet.</td>
        </tr>
      `;
  }
  ...
  function renderFinancialVisuals(data, analytics = buildFinancialAnalytics(data)) {
    ...
    if (overviewMonthSalesNode) overviewMonthSalesNode.textContent = formatCurrency(analytics.currentMonthGrossSales);
    if (overviewMonthNetNode) overviewMonthNetNode.textContent = formatCurrency(analytics.currentMonthNetRevenue);
    if (overviewPipelineNode) overviewPipelineNode.textContent = formatCurrency(analytics.pendingRevenue);
    if (overviewPayoutNode) overviewPayoutNode.textContent = formatCurrency(analytics.coachPayoutsPayable);
  ...
  function renderFinanceCustomerDirectory(data, analytics) {
    ...
    financeCustomerRowsNode.innerHTML = (data.clients || []).length
      ...
      : `
        <tr>
          <td colspan="6">No customer data yet.</td>
        </tr>
      `;
  }
  ...
  function renderFinancials(data, analytics = buildFinancialAnalytics(data)) {
    ...
    financialRowsNode.innerHTML = transactionOrders.length
      ...
      : `
        <tr>
          <td colspan="9">No orders or payments yet.</td>
        </tr>
      `;
  ...
  function renderPackagePurchases(data) {
    ...
    packagePurchaseRowsNode.innerHTML = data.clientPackages.length
      ...
      : `
        <tr>
          <td colspan="7">No package purchases yet.</td>
        </tr>
      `;
  }
