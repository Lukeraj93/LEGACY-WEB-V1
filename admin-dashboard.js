(function initAdminDashboard() {
  ...
  const coachReviewPaginationNode = document.getElementById("admin-coach-review-pagination");
  ...
  const newsletterGeneratorFormNode = document.getElementById("admin-newsletter-generator-form");
  const newsletterGeneratorPresetsNode = document.getElementById("admin-newsletter-generator-presets");
  const newsletterGeneratorPresetNodes = Array.from(document.querySelectorAll("[data-newsletter-generator-preset]"));
  const newsletterGeneratorAudienceNode = document.getElementById("admin-newsletter-generator-audience");
  const newsletterGeneratorTypeNode = document.getElementById("admin-newsletter-generator-type");
  const newsletterGeneratorTopicNode = document.getElementById("admin-newsletter-generator-topic");
  const newsletterGeneratorToneNode = document.getElementById("admin-newsletter-generator-tone");
  const newsletterGeneratorGoalNode = document.getElementById("admin-newsletter-generator-goal");
  const newsletterGeneratorEvidenceStyleNode = document.getElementById("admin-newsletter-generator-evidence-style");
  const newsletterGeneratorPromptNode = document.getElementById("admin-newsletter-generator-prompt");
  const newsletterGenerateButtonNode = document.getElementById("admin-newsletter-generate");
  const newsletterGeneratorFeedbackNode = document.getElementById("admin-newsletter-generator-feedback");
  const newsletterOpenManualButtonNode = document.getElementById("admin-newsletter-open-manual");
  const newsletterCloseManualButtonNode = document.getElementById("admin-newsletter-close-manual");
  const newsletterManualShellNode = document.getElementById("admin-newsletter-manual-shell");
  const newsletterPanelTabsNode = document.getElementById("admin-newsletter-tabs");
  const newsletterPanelTabNodes = Array.from(document.querySelectorAll("[data-newsletter-panel-target]"));
  const newsletterPanelNodes = Array.from(document.querySelectorAll("[data-newsletter-panel]"));
  const newsletterPanelKickerNode = document.getElementById("admin-newsletter-panel-kicker");
  const newsletterPanelTitleNode = document.getElementById("admin-newsletter-panel-title");
  const newsletterPanelDescriptionNode = document.getElementById("admin-newsletter-panel-description");
  const newsletterPanelTipNode = document.getElementById("admin-newsletter-panel-tip");
  ...
  function filterOperationalProfiles(records) {
    return (records || []).filter((record) => !matchesNonOperationalName(record?.display_name));
  }

  function filterOperationalLeads(records) {
    return (records || []).filter((record) => !matchesNonOperationalName(record?.full_name) && !matchesNonOperationalEmail(record?.email));
  }

  function filterOperationalActivationCodes(records) {
    return (records || []).filter((record) => !matchesNonOperationalName(record?.recipient_name) && !matchesNonOperationalEmail(record?.recipient_email));
  }
  ...
  const NEWSLETTER_PANEL_COPY = {
    playbook: {
      kicker: "Start here",
      title: "Overview first",
      description: "Use this like your weekly send brief. It tells the admin team what kind of issue to write, what tone to use, and what members should feel after they open it.",
      tip: "Best next move: once the weekly angle is clear, go straight into Compose and write the issue while the message is still focused.",
    },
    compose: {
      kicker: "Build the issue",
      title: "Write what members need this week",
      description: "This is where you turn the weekly plan into a real member email: subject line, update, one useful coaching point, and a clear next action.",
      tip: "Best next move: save once the structure is locked, then open Preview to see the exact email before anyone receives it.",
    },
    preview: {
      kicker: "Proof it properly",
      title: "Preview the exact send",
      description: "Use this panel like a final check before delivery. Confirm the subject, the tone, the CTA, and how the full issue reads in one uninterrupted flow.",
      tip: "Best next move: if anything feels off, go back to Compose, tighten it, and refresh the preview again before testing.",
    },
    delivery: {
      kicker: "Send with control",
      title: "Test first, then send live",
      description: "This is the release panel. Use it to verify the audience, send a proof email, then push the final issue once the preview and copy are locked.",
      tip: "Best next move: send a test to yourself or the ops lead first, then use the campaign history below to confirm delivery quality after the send.",
    },
  };
  const NEWSLETTER_GENERATOR_PRESETS = {
    member_education: {
      audienceSegment: "active_members",
      newsletterType: "educational_deep_dive",
      topic: "strength_training",
      tone: "professional",
      primaryGoal: "educate",
      evidenceStyle: "education_heavy",
      prompt: "Write for active LEGACY+ clients. Keep it practical, coach-led, and useful for busy adults. Teach one concept clearly, show how it applies this week, and end by pushing members to book sessions and stay consistent.",
      feedback: "Member education preset loaded. This is best for useful client learning that still leads back into booking, consistency, and coaching trust.",
    },
    coach_education: {
      audienceSegment: "active_coaches",
      newsletterType: "coach_development",
      topic: "coach_spotlight",
      tone: "direct",
      primaryGoal: "educate",
      evidenceStyle: "light",
      prompt: "Write for active coaches inside LEGACY+. Focus on better delivery, coaching judgment, communication, retention, or standards on the floor. Make it useful before the next session, not abstract.",
      feedback: "Coach education preset loaded. This is best for internal coaching standards, cueing, retention, and delivery quality.",
    },
    re_engagement: {
      audienceSegment: "lapsed_members",
      newsletterType: "update_announcement",
      topic: "habit_systems",
      tone: "community",
      primaryGoal: "re_engage",
      evidenceStyle: "none",
      prompt: "Write for people who have gone quiet. Keep the tone warm, human, and low-pressure. Remind them how to restart, reduce friction, and make one simple move back into training this week.",
      feedback: "Re-engagement preset loaded. This is best when the goal is to get quiet clients or lapsed members moving again without sounding pushy.",
    },
    promo_campaign: {
      audienceSegment: "active_members",
      newsletterType: "update_announcement",
      topic: "offer_or_campaign",
      tone: "premium",
      primaryGoal: "promote",
      evidenceStyle: "none",
      prompt: "Write around one clear offer or campaign. Keep it premium and high-conviction without sounding cheap. Make the value obvious fast, explain what action to take, and give a clean CTA.",
      feedback: "Promo preset loaded. This is best for launches, offers, limited-time pushes, or high-visibility campaigns.",
    },
    operational_update: {
      audienceSegment: "active_members",
      newsletterType: "update_announcement",
      ...
      tone: "direct",
      primaryGoal: "announce",
      evidenceStyle: "none",
      prompt: "Write a clear operational update. Prioritize schedule changes, team announcements, class changes, or service notes. Keep it concise and easy to scan so members know exactly what changed and what to do next.",
      feedback: "Operational update preset loaded. This is best for schedule changes, coach arrivals, closures, or logistics that members need quickly.",
    },
  };
  ...
  const dashboardState = {
    ...
    newsletterCapabilities: null,
    newsletterLoaded: false,
    newsletterLoading: false,
    newsletterDirty: false,
    newsletterCustomSections: [],
    newsletterPanel: "playbook",
    newsletterManualOpen: false,
    paginatedTables: {
      coachReviewQueue: {
        page: 1,
        pageSize: 10,
  ...
  function activateNewsletterPanel(panelId, options = {}) {
    if (!isNewsletterWorkspacePage()) {
      return;
    }

    const allowedPanels = new Set(["playbook", "compose", "preview", "delivery"]);
    const nextPanel = allowedPanels.has(panelId) ? panelId : "playbook";
    dashboardState.newsletterPanel = nextPanel;

    newsletterPanelTabNodes.forEach((button) => {
      const isActive = button.getAttribute("data-newsletter-panel-target") === nextPanel;
      button.classList.toggle("is-active", isActive);
      button.setAttribute("aria-pressed", String(isActive));
      if (button.classList.contains("newsletter-workflow-step")) {
        button.setAttribute("aria-current", isActive ? "step" : "false");
      }
    });

    newsletterPanelNodes.forEach((panel) => {
      const isActive = panel.getAttribute("data-newsletter-panel") === nextPanel;
      panel.hidden = !isActive;
      panel.classList.toggle("is-active", isActive);
    });

    renderNewsletterPanelGuide(nextPanel);
  ...
  function bindNewsletterWorkspaceNavigation() {
    if (!isNewsletterWorkspacePage() || !newsletterPanelTabsNode) {
      return;
    }

    document.addEventListener("click", (event) => {
      const button = event.target.closest("[data-newsletter-panel-target]");
      if (!(button instanceof HTMLButtonElement)) {
        return;
      }

      activateNewsletterPanel(button.getAttribute("data-newsletter-panel-target") || "playbook");
    });

    const hashPanel = String(window.location.hash || "").replace(/^#/, "").trim();
    activateNewsletterPanel(hashPanel || dashboardState.newsletterPanel || "playbook", { updateHash: Boolean(hashPanel) });
  }

  function renderNewsletterPanelGuide(panelId) {
    const copy = NEWSLETTER_PANEL_COPY[panelId] || NEWSLETTER_PANEL_COPY.playbook;
    if (newsletterPanelKickerNode) newsletterPanelKickerNode.textContent = copy.kicker;
    if (newsletterPanelTitleNode) newsletterPanelTitleNode.textContent = copy.title;
    if (newsletterPanelDescriptionNode) newsletterPanelDescriptionNode.textContent = copy.description;
    if (newsletterPanelTipNode) newsletterPanelTipNode.textContent = copy.tip;
  }
  ...
  function setNewsletterGeneratorFeedback(message, isError) {
    if (!newsletterGeneratorFeedbackNode) {
      return;
    }

    newsletterGeneratorFeedbackNode.textContent = message || "";
    newsletterGeneratorFeedbackNode.style.color = isError ? "#ffb3b3" : "";
  }
  ...
  function setNewsletterBusy(isBusy) {
    if (newsletterGenerateButtonNode) {
      newsletterGenerateButtonNode.disabled = Boolean(isBusy) || !Boolean(dashboardState.newsletterCapabilities?.aiDrafting);
    }
    ...
    if (newsletterOpenManualButtonNode) {
      newsletterOpenManualButtonNode.disabled = Boolean(isBusy);
    }

    if (newsletterCloseManualButtonNode) {
      newsletterCloseManualButtonNode.disabled = Boolean(isBusy);
    }
  }
  ...
  function buildQaAccountSummaryMarkup(status) {
    ...
    const coachSummary = coach.exists
      ? `${renderAdminStatusPill(coach.profileStatus || "active", "account")} <span>${escapeHtml(normalizeStatusLabel(coach.role || "coach"))}</span>`
      : renderStatusPill("Not Provisioned", "gray");
    const clientSummary = client.exists
      ? `${renderAdminStatusPill(client.profileStatus || "active", "account")} <span>${escapeHtml(normalizeStatusLabel(client.role || "client"))}</span>`
      : renderStatusPill("Not Provisioned", "gray");
    const assignmentSummary = assignment.linked
      ? `${renderAdminStatusPill("linked", "assignment", "Linked")} <span>${escapeHtml(formatDate(assignment.assignedAt))}</span>`
      : coach.exists && client.exists
        ? renderAdminStatusPill("accounts_exist_but_are_not_linked", "assignment", "Not Linked")
        : renderAdminStatusPill("waiting_for_both_qa_accounts", "assignment", "Waiting");

    return `
      <article class="dashboard-badge" data-crm-tone="${escapeHtml(coach.exists ? "info" : "neutral")}">
        <strong>QA Coach</strong>
        <p>${coachSummary}</p>
      </article>
      <article class="dashboard-badge" data-crm-tone="${escapeHtml(client.exists ? "info" : "neutral")}">
        <strong>QA Client</strong>
        <p>${clientSummary}</p>
      </article>
      <article class="dashboard-badge" data-crm-tone="${escapeHtml(assignment.linked ? "success" : coach.exists && client.exists ? "warning" : "neutral")}">
  ...
  function applyNewsletterCampaignToForm(campaign) {
    ...
    if (newsletterGeneratorAudienceNode) newsletterGeneratorAudienceNode.value = source.audienceSegment || "active_members";
    if (newsletterGeneratorTypeNode) newsletterGeneratorTypeNode.value = source.newsletterType || "update_announcement";
  ...
  function getNewsletterGeneratorValues() {
    return {
      audienceSegment: String(newsletterGeneratorAudienceNode?.value || newsletterAudienceSegmentNode?.value || "active_members").trim(),
      newsletterType: String(newsletterGeneratorTypeNode?.value || newsletterTypeNode?.value || "update_announcement").trim(),
      topic: String(newsletterGeneratorTopicNode?.value || "strength_training").trim(),
      tone: String(newsletterGeneratorToneNode?.value || "professional").trim(),
      primaryGoal: String(newsletterGeneratorGoalNode?.value || "educate").trim(),
      evidenceStyle: String(newsletterGeneratorEvidenceStyleNode?.value || "light").trim(),
      prompt: String(newsletterGeneratorPromptNode?.value || "").trim(),
    };
  }

  function syncNewsletterGeneratorPresetButtons(activeKey) {
    newsletterGeneratorPresetNodes.forEach((button) => {
      const isActive = button.getAttribute("data-newsletter-generator-preset") === activeKey;
      button.classList.toggle("is-active", isActive);
      button.setAttribute("aria-pressed", String(isActive));
    });
  }

  function applyNewsletterGeneratorPreset(presetKey) {
    const preset = NEWSLETTER_GENERATOR_PRESETS[presetKey];
    if (!preset) {
      return;
    }

    if (newsletterGeneratorAudienceNode) newsletterGeneratorAudienceNode.value = preset.audienceSegment;
    if (newsletterGeneratorTypeNode) newsletterGeneratorTypeNode.value = preset.newsletterType;
    if (newsletterGeneratorTopicNode) newsletterGeneratorTopicNode.value = preset.topic;
    if (newsletterGeneratorToneNode) newsletterGeneratorToneNode.value = preset.tone;
    if (newsletterGeneratorGoalNode) newsletterGeneratorGoalNode.value = preset.primaryGoal;
    if (newsletterGeneratorEvidenceStyleNode) newsletterGeneratorEvidenceStyleNode.value = preset.evidenceStyle;
    if (newsletterGeneratorPromptNode) newsletterGeneratorPromptNode.value = preset.prompt;
    if (newsletterAudienceSegmentNode) newsletterAudienceSegmentNode.value = preset.audienceSegment;
    if (newsletterTypeNode) newsletterTypeNode.value = preset.newsletterType;

    syncNewsletterGeneratorPresetButtons(presetKey);
    dashboardState.newsletterDirty = true;
    if (dashboardState.newsletterCapabilities?.aiDrafting) {
      setNewsletterGeneratorFeedback(preset.feedback, false);
    } else {
      setNewsletterGeneratorFeedback(
        `${preset.feedback} AI drafting is not configured yet, so use the preset as a starting brief and open the manual editor to finish the issue.`,
        true
      );
    }
  }

  function setNewsletterManualEditorVisible(nextVisible) {
    const visible = Boolean(nextVisible);
    dashboardState.newsletterManualOpen = visible;
    if (newsletterManualShellNode) {
      newsletterManualShellNode.hidden = !visible;
    }
    if (newsletterOpenManualButtonNode) {
      newsletterOpenManualButtonNode.textContent = visible ? "Manual Editor Open" : "Open Manual Editor";
      newsletterOpenManualButtonNode.setAttribute("aria-expanded", String(visible));
    }
  }

  function applyNewsletterCapabilities(capabilities) {
    dashboardState.newsletterCapabilities = capabilities || dashboardState.newsletterCapabilities || { aiDrafting: false };
    const aiDraftingEnabled = Boolean(dashboardState.newsletterCapabilities?.aiDrafting);
    if (newsletterGenerateButtonNode) {
      newsletterGenerateButtonNode.disabled = dashboardState.newsletterLoading ? true : !aiDraftingEnabled;
      newsletterGenerateButtonNode.title = aiDraftingEnabled ? "" : "OpenAI drafting is not configured yet.";
    }

    if (!aiDraftingEnabled) {
      setNewsletterGeneratorFeedback(
        "AI drafting is not configured yet. You can still use the manual editor, or add OPENAI_API_KEY in Netlify to enable one-click draft generation.",
        true
      );
    } else if (!dashboardState.newsletterLoading) {
      setNewsletterGeneratorFeedback(
        "Choose the audience, topic, and tone, then generate a draft. We will prefill the issue and take you straight into preview.",
        false
      );
    }
  }
  ...
  function buildNewsletterSummaryMarkup(status, analytics) {
    ...
    return `
      <article class="dashboard-badge" data-crm-tone="${escapeHtml(status?.enabled ? "success" : "warning")}">
        <strong>Automation</strong>
        <p>${renderAdminStatusPill(status?.enabled ? "approved" : "pending", "generic", enabledLabel)}</p>
      </article>
      <article class="dashboard-badge" data-crm-tone="info">
        <strong>Audience</strong>
        <p>${escapeHtml(`${Number(status?.audienceCount || 0)} • ${status?.audienceSegmentLabel || "Active Members"}`)}</p>
      </article>
      <article class="dashboard-badge" data-crm-tone="${escapeHtml(status?.enabled ? "warning" : "neutral")}">
        <strong>Next Run</strong>
        <p>${escapeHtml(nextRunLabel)}</p>
      </article>
      <article class="dashboard-badge" data-crm-tone="${escapeHtml(latestDispatch && latestDispatch.openRate >= 0.35 ? "success" : "info")}">
        <strong>Latest Open Rate</strong>
        <p>${escapeHtml(latestDispatch ? formatPercent(latestDispatch.openRate * 100, 0) : "0%")}</p>
      </article>
      <article class="dashboard-badge" data-crm-tone="${escapeHtml(latestDispatch && latestDispatch.clickRate >= 0.03 ? "success" : "purple")}">
  ...
  async function loadNewsletterCampaign(options) {
    ...
    try {
      ...
      applyNewsletterCapabilities(payload?.capabilities || null);
  ...
  async function handleNewsletterPreview() {
    ...
    activateNewsletterPanel("preview");
    dashboardState.newsletterLoading = true;
    setNewsletterBusy(true);
  ...
  async function handleNewsletterGenerateDraft(event) {
    event?.preventDefault?.();
    if (dashboardState.newsletterLoading) {
      return;
    }

    const generator = getNewsletterGeneratorValues();
    if (!generator.topic && !generator.prompt) {
      setNewsletterGeneratorFeedback("Pick a topic or add a short prompt before generating the newsletter.", true);
      return;
    }

    if (newsletterAudienceSegmentNode) {
      newsletterAudienceSegmentNode.value = generator.audienceSegment;
    }
    if (newsletterTypeNode) {
      newsletterTypeNode.value = generator.newsletterType;
    }

    activateNewsletterPanel("compose");
    dashboardState.newsletterLoading = true;
    setNewsletterBusy(true);
    setNewsletterGeneratorFeedback("Generating a full newsletter draft from your brief...", false);
    setNewsletterFeedback("Generating an AI newsletter draft...", false);

    try {
      const payload = await requestNewsletterCampaign("POST", {
        action: "generate_draft",
        generator,
        campaign: getNewsletterFormValues(),
      });
      dashboardState.newsletterCampaign = payload?.campaign || dashboardState.newsletterCampaign;
      dashboardState.newsletterStatus = payload?.status || dashboardState.newsletterStatus;
      dashboardState.newsletterPreview = payload?.preview || dashboardState.newsletterPreview;
      dashboardState.newsletterAnalytics = payload?.analytics || dashboardState.newsletterAnalytics;
      applyNewsletterCapabilities(payload?.capabilities || null);
      ...
      dashboardState.newsletterDirty = false;
      renderNewsletterCampaign();
      setNewsletterGeneratorFeedback("Draft generated. Review the preview next, then send a test before publishing.", false);
      setNewsletterFeedback(payload?.message || "AI draft generated. Review the preview next.", false);
      activateNewsletterPanel("preview");
    } catch (error) {
      setNewsletterGeneratorFeedback(error?.message || "Unable to generate the AI draft right now.", true);
      setNewsletterFeedback(error?.message || "Unable to generate the AI draft right now.", true);
    } finally {
      dashboardState.newsletterLoading = false;
      setNewsletterBusy(false);
    }
  ...
  async function handleNewsletterSendNow() {
    ...
    activateNewsletterPanel("delivery");
    dashboardState.newsletterLoading = true;
    setNewsletterBusy(true);
  ...
  function renderStatusPill(label, tone = "gray") {
    const safeTone = ["red", "orange", "yellow", "green", "blue", "purple", "gray"].includes(tone)
      ? tone
      : "gray";
    return `<span class="crm-status-pill crm-status-pill--${safeTone}">${escapeHtml(label)}</span>`;
  }

  function getAdminStatusTone(status, domain = "generic") {
    const key = normalizeStatusKey(status);
    const area = String(domain || "generic").toLowerCase();
    const oneOf = (...values) => values.includes(key);

    if (area === "coach_review" || area === "coach_roster") {
      if (oneOf("overdue")) return "red";
      if (oneOf("due_soon")) return "yellow";
      if (oneOf("promotion_ready")) return "purple";
      if (oneOf("kpi_green", "on_track")) return "green";
      if (oneOf("needs_review_date")) return "orange";
      if (key.startsWith("review_")) return "yellow";
      if (oneOf("active")) return "blue";
      if (oneOf("inactive", "ended", "archived")) return "gray";
    }

    if (area === "lead") {
      if (oneOf("overdue", "overdue_follow_up")) return "red";
      if (oneOf("new")) return "blue";
      if (oneOf("contacted")) return "yellow";
      if (oneOf("qualified")) return "purple";
      if (oneOf("converted")) return "green";
      if (oneOf("ghosted", "lost")) return "gray";
    }

    if (area === "auth_code") {
      if (oneOf("expired")) return "red";
      if (oneOf("pending", "queued")) return "orange";
      if (oneOf("used")) return "green";
      if (oneOf("revoked", "cancelled", "inactive")) return "gray";
      if (oneOf("active")) return "blue";
    }

    if (area === "reward_approval") {
      if (oneOf("rejected")) return "red";
      if (oneOf("pending")) return "orange";
      if (oneOf("approved")) return "green";
    }

    if (area === "payment") {
      if (oneOf("failed", "declined", "expired")) return "red";
      if (oneOf("pending", "pending_payment", "processing", "authorized")) return "orange";
      if (oneOf("partially_refunded")) return "yellow";
      if (oneOf("paid", "succeeded", "captured")) return "green";
      if (oneOf("refunded", "cancelled", "voided")) return "gray";
    }
    ...
    if (area === "package") {
      ...
      if (oneOf("pending")) return "orange";
      if (oneOf("active")) return "blue";
      if (oneOf("completed")) return "green";
      if (oneOf("cancelled", "expired", "ended")) return "gray";
    }

    if (area === "payout") {
      if (oneOf("rejected")) return "red";
      if (oneOf("pending")) return "orange";
      if (oneOf("approved")) return "blue";
      if (oneOf("paid")) return "green";
      if (oneOf("cancelled")) return "gray";
    }

    if (area === "notification_status") {
      if (oneOf("new", "unread")) return "orange";
      if (oneOf("read")) return "gray";
    }

    if (area === "notification_category") {
      if (["security", "refund", "failure", "error", "critical"].includes(key)) return "red";
      if (["payment", "payments", "finance", "billing", "order"].includes(key)) return "orange";
      if (["coach_review", "follow_up", "review", "reminder"].includes(key)) return "yellow";
      if (["reward", "xp", "codex", "newsletter", "campaign", "qa"].includes(key)) return "purple";
      if (["lead", "inquiry", "consult", "system", "client", "coach"].includes(key)) return "blue";
    }

    if (area === "account") {
      if (oneOf("suspended", "blocked")) return "red";
      if (oneOf("pending")) return "orange";
      if (oneOf("active")) return "blue";
      if (oneOf("inactive", "ended", "deleted")) return "gray";
    }

    if (area === "waiver") {
      if (oneOf("missing_waiver")) return "red";
      if (oneOf("signed")) return "green";
      if (oneOf("not_required")) return "gray";
    }

    if (area === "engagement") {
      if (oneOf("active_package")) return "blue";
      if (oneOf("no_package_yet")) return "yellow";
      if (oneOf("past_package_only")) return "gray";
    }

    if (area === "assignment") {
      if (oneOf("linked", "active")) return "green";
      if (oneOf("accounts_exist_but_are_not_linked", "not_linked")) return "orange";
      if (oneOf("waiting_for_both_qa_accounts", "not_provisioned")) return "gray";
    }

    if (key.includes("follow_up") || key.includes("review")) return "yellow";
    if (key.includes("convert")) return "green";
    if (key.includes("qualif")) return "purple";
    if (key.includes("lead") || key.includes("contact")) return "blue";
    if (key.includes("payment") || key.includes("billing") || key.includes("order")) return "orange";
    if (key.includes("newsletter") || key.includes("reward") || key.includes("codex") || key.includes("campaign")) return "purple";

    if (oneOf("critical", "danger", "overdue", "failed", "missing", "rejected", "expired", "blocked")) return "red";
    if (oneOf("pending", "requested", "queued", "open")) return "orange";
    if (oneOf("warning", "due_soon", "review")) return "yellow";
    if (oneOf("promotion_ready", "qualified", "newsletter", "codex", "qa")) return "purple";
    if (oneOf("approved", "paid", "completed", "signed", "healthy", "green", "success")) return "green";
    if (oneOf("active", "live", "new", "info", "connected")) return "blue";
    if (oneOf("inactive", "cancelled", "archived", "read", "lost", "ghosted", "not_required", "refunded")) return "gray";

    return "gray";
  }

  function renderAdminStatusPill(status, domain = "generic", label) {
    const resolvedLabel = label || normalizeStatusLabel(status || "pending");
    return renderStatusPill(resolvedLabel, getAdminStatusTone(status || resolvedLabel, domain));
  }
            ...
            .map(
              (row) => `
                <tr>
                  ...
                  <td>${renderAdminStatusPill(row.status || "unknown")}</td>
  ...
  function notifyAdminShellHydration(data, access) {
    document.dispatchEvent(
      new CustomEvent("legacy:admin-shell-hydrate", {
        detail: {
          access: access || dashboardState.access || null,
          data: data || dashboardState.lastData || buildEmptyDashboardData(),
        },
      })
    );
  }
  ...
  async function fetchDashboardDataFromServer(options = {}) {
    ...
    const params = new URLSearchParams({
      scope: "admin",
      dashboardPage: DASHBOARD_PAGE_KEY,
    });
    if (options?.variant) {
  ...
  async function fetchPaginatedDashboardResource(resource, page, pageSize) {
    const accessToken = await getAccessToken();
    const params = new URLSearchParams({
      scope: "admin",
      dashboardPage: DASHBOARD_PAGE_KEY,
      ...
      pageSize: String(Math.max(1, Number(pageSize || 10))),
    });
    const response = await window.fetch(`/.netlify/functions/load-dashboard-data?${params.toString()}`, {
      method: "GET",
      headers: {
    ...
    const payload = await response.json().catch(() => ({}));
    if (!response.ok || !payload?.data) {
      throw new Error(payload?.error || "Unable to load the paginated dashboard queue right now.");
    }
    return payload.data;
  }
  ...
  async function fetchDashboardDataDirect() {
    ...
    if (DASHBOARD_PAGE_KEY === "clients") {
      ...
      const coachIds = new Set(coaches.map((item) => item.id));
      const clientIds = new Set(clients.map((item) => item.id));
      const rewards = (rewardsResponse.data || []).filter((reward) => clientIds.has(reward?.client_id));
      ...
      syncDashboardStateFromData({
        coaches,
        clients,
        assignments: (assignmentsResponse.data || []).filter((assignment) => coachIds.has(assignment?.coach_id) && clientIds.has(assignment?.client_id)),
        rewards,
        plannerRewardEvents,
        notifications: filterOperationalNotifications(notificationsResponse.data || []),
        activationCodes: filterOperationalActivationCodes(activationCodesResponse.data || []),
      });

      return {
        access,
        coaches,
        coachProfiles: (coachProfilesResponse.data || []).filter((profile) => coachIds.has(profile?.id)),
        clients,
        clientProfiles: (clientProfilesResponse.data || []).filter((profile) => clientIds.has(profile?.id)),
        assignments: (assignmentsResponse.data || []).filter((assignment) => coachIds.has(assignment?.coach_id) && clientIds.has(assignment?.client_id)),
        clientPackages: (packagesResponse.data || []).filter((pkg) => clientIds.has(pkg?.client_id)),
        rewards,
        plannerRewardEvents,
        notifications: filterOperationalNotifications(notificationsResponse.data || []),
        activationCodes: filterOperationalActivationCodes(activationCodesResponse.data || []),
    ...
    if (DASHBOARD_PAGE_KEY === "leads") {
      ...
      const leadActivities = (leadActivitiesResponse.data || []).filter((activity) => allowedLeadIds.has(activity?.lead_id));
    ...
    if (DASHBOARD_PAGE_KEY === "financials") {
      ...
      const clients = filterOperationalProfiles(clientsResponse.data || []);
      const clientIds = new Set(clients.map((item) => item.id));
      const orders = (ordersResponse.data || []).filter((order) => clientIds.has(order?.client_id));
      ...
      return {
        ...
        clientPackages: (packagesResponse.data || []).filter((pkg) => clientIds.has(pkg?.client_id)),
        orders,
        sessions: (sessionsResponse.data || []).filter((session) =>
          (packagesResponse.data || []).some((pkg) => clientIds.has(pkg?.client_id) && pkg?.id === session?.client_package_id)
        ),
        payments: paymentsResponse.data || [],
        orderItems: orderItemsResponse.data || [],
        commissions: (commissionsResponse.data || []).filter((record) => coaches.some((coach) => coach.id === record?.coach_id)),
        bookingRequests: (bookingRequestsResponse.data || []).filter((request) => clientIds.has(request?.client_id)),
    ...
    const clientIds = new Set(clients.map((item) => item.id));
    const coachIds = new Set(coaches.map((item) => item.id));
    const packageRows = (packagesResponse.data || []).filter((pkg) => clientIds.has(pkg?.client_id));
    ...
    return {
      ...
      operatingCostEntries: dashboardState.lastData?.operatingCostEntries || [],
      operatingCostSummary: dashboardState.lastData?.operatingCostSummary || null,
      orders: orders.filter((order) => clientIds.has(order?.client_id)),
      sessions: sessions.filter((session) => packageIds.has(session?.client_package_id)),
      payments: paymentsResponse.data || [],
      orderItems: orderItemsResponse.data || [],
      commissions: (commissionsResponse.data || []).filter((record) => coaches.some((coach) => coach.id === record?.coach_id)),
      bookingRequests: (bookingRequestsResponse.data || []).filter((request) => clientIds.has(request?.client_id)),
      actorProfiles: (actorProfilesResponse.data || []).filter((profile) => coachIds.has(profile?.id) || clientIds.has(profile?.id) || profile?.id === access.user.id),
      currentMonthStart,
    };
  }
  ...
  function renderCoachManagement(data) {
    ...
    if (coachSelectionSummaryNode) {
      ...
      coachSelectionSummaryNode.innerHTML = `
        ...
        <div class="chips">
          ${renderAdminStatusPill(coach.status || "pending", "account")}
          ...
          ${renderAdminStatusPill(
            profile?.review_due_at ? `review_${formatDate(profile.review_due_at)}` : "needs_review_date",
            "coach_roster",
            profile?.review_due_at ? `Review ${formatDate(profile.review_due_at)}` : "No review date"
          )}
        </div>
  ...
  function getCoachReviewQueueState() {
    return dashboardState.paginatedTables.coachReviewQueue;
  }

  function buildDashboardPaginationMarkup(state, label) {
    const safeState = state || {};
    const totalPages = Math.max(1, Number(safeState.totalPages || 1));
    const currentPage = Math.min(Math.max(1, Number(safeState.page || 1)), totalPages);
    ...
    const buttonsMarkup = uniquePageNumbers
      .map((pageNumber) => `
        <button
          class="dashboard-table-pagination__button${pageNumber === currentPage ? " is-active" : ""}"
          type="button"
          data-pagination-target="${escapeHtml(label)}"
    ...
    return `
      <p class="dashboard-table-pagination__meta">Page ${currentPage} of ${totalPages} • ${totalCount} total</p>
      <div class="dashboard-table-pagination__controls">
        <button
          class="dashboard-table-pagination__button dashboard-table-pagination__button--nav"
          type="button"
          data-pagination-target="${escapeHtml(label)}"
  ...
  function renderCoachReviewPagination(state) {
    if (!coachReviewPaginationNode) {
      return;
    }

    const markup = buildDashboardPaginationMarkup(state, "coach-review");
    coachReviewPaginationNode.hidden = !markup;
    coachReviewPaginationNode.innerHTML = markup;
  }

  async function loadCoachReviewQueuePage(page, options = {}) {
    if (!isHomeWorkspacePage()) {
      return;
    }

    const queueState = getCoachReviewQueueState();
    ...
    queueState.error = "";
    queueState.page = Math.max(1, Number(page || queueState.page || 1));
    renderCoachReviewPagination(queueState);

    try {
      const payload = await fetchPaginatedDashboardResource("coach_review_queue", queueState.page, queueState.pageSize);
      if (payload?.resource !== "coach_review_queue") {
        throw new Error("Unexpected review queue response.");
      }

      queueState.page = Number(payload.pagination?.page || queueState.page || 1);
      queueState.pageSize = Number(payload.pagination?.pageSize || queueState.pageSize || 10);
      queueState.totalPages = Number(payload.pagination?.totalPages || 1);
      queueState.totalCount = Number(payload.pagination?.totalCount || 0);
      queueState.rows = Array.isArray(payload.rows) ? payload.rows : [];
      queueState.initialized = true;
    } catch (error) {
      queueState.error = error?.message || "Unable to load the coach review queue.";
      queueState.initialized = true;
      if (options.showStatus !== false) {
        setStatus(queueState.error, true);
      }
    } finally {
      queueState.loading = false;
      renderCoachReviewQueue(dashboardState.lastData || buildEmptyDashboardData());
    }
  }

  function ensureHomePaginationState(options = {}) {
    if (!isHomeWorkspacePage()) {
      return;
    }

    const queueState = getCoachReviewQueueState();
    if (options.force || !queueState.initialized) {
      void loadCoachReviewQueuePage(queueState.page || 1, {
        showStatus: false,
      });
    }
  }
  ...
  function renderCoachReviewQueue(data) {
    ...
    const queueState = getCoachReviewQueueState();
    const fallbackRows = coachReviewSummary(data).sort((left, right) => left.reviewKey - right.reviewKey);
    const visibleRows = Array.isArray(queueState.rows) ? queueState.rows : fallbackRows;
    const overdueCount = fallbackRows.filter((row) => row.isOverdue).length;
    const dueSoonCount = fallbackRows.filter((row) => row.isDueSoon).length;
    const promotionReadyCount = fallbackRows.filter((row) => row.promotionReady).length;
    const greenCount = fallbackRows.filter((row) => row.allKpisChecked).length;
    ...
    setTone(overdueNode, overdueCount ? "alert" : "neutral");
    setTone(dueSoonNode, dueSoonCount ? "caution" : "neutral");
    setTone(promotionReadyNode, promotionReadyCount ? "purple" : "neutral");
    setTone(greenNode, greenCount ? "success" : "neutral");

    coachReviewRowsNode.innerHTML = visibleRows.length
      ? visibleRows
          .map((row) => `
            <tr>
              ...
              <td>${escapeHtml(
                !row.reviewLabel || row.reviewLabel === "Not scheduled"
                  ? "Not scheduled"
                  : String(row.reviewLabel).includes("T") || /^\d{4}-\d{2}-\d{2}/u.test(String(row.reviewLabel))
                    ? formatDate(row.reviewLabel)
                    : row.reviewLabel
              )}</td>
              <td>${renderAdminStatusPill(
                row.status || (row.isOverdue ? "overdue" : row.isDueSoon ? "due_soon" : row.promotionReady ? "promotion_ready" : "on_track"),
                "coach_review",
                row.status === "overdue"
                  ? "Overdue"
                  : row.status === "due_soon"
                    ? "Due Soon"
                    : row.status === "promotion_ready"
                      ? "Promotion Ready"
                      : "On Track"
              )}</td>
            </tr>
    ...
    if (!Array.isArray(queueState.rows)) {
      queueState.totalCount = fallbackRows.length;
      queueState.totalPages = Math.max(1, Math.ceil(fallbackRows.length / queueState.pageSize));
    }
    renderCoachReviewPagination(queueState);
  }
          ...
          .map((lead) => {
            ...
            return `
              <tr>
                ...
                <td>${renderAdminStatusPill(lead.status || "new", "lead")}</td>
  ...
  function renderHero(data, analytics = buildFinancialAnalytics(data)) {
    ...
    setTone(netRevenueNode, analytics.netRevenue > 0 ? "info" : "neutral");
    setTone(payoutsNode, analytics.coachPayoutsPayable > 0 ? "warning" : "neutral");
    setTone(liabilityNode, analytics.unusedLiability > 0 ? "caution" : "neutral");
    setTone(conversionNode, analytics.leadConversionRate > 0 ? "success" : "info");
    setTone(activeClientsNode, analytics.activeClientCount > 0 ? "info" : "neutral");
          ...
          .map((coach) => {
            ...
            return `
              <tr>
                ...
                <td>${renderAdminStatusPill(rowStatus, "coach_roster", rowStatus)}</td>
          ...
          .map((client) => {
            ...
            return `
              <tr>
                ...
                <td>${renderAdminStatusPill(
                  engagementStatus === "Active package"
                    ? "active_package"
                    : engagementStatus === "Past package only"
                      ? "past_package_only"
                      : "no_package_yet",
                  "engagement",
                  engagementStatus
          ...
          .map((record) => {
            ...
            return `
              <tr>
                ...
                <td>${renderAdminStatusPill(record.status || "active", "auth_code")}</td>
          ...
          .map((activity) => {
            ...
            return `
              <tr>
                ...
                <td>${renderAdminStatusPill(activity.activity_type || "system", "notification_category")}</td>
          ...
          .map((reward) => {
            ...
            const sourceLabel = plannerRewardEvent
              ...
              : action
                ? "Codex Reward"
                : "Manual Reward";
            const sourceTone = plannerRewardEvent ? "info" : action ? "purple" : "warning";
            ...
            return `
              <tr>
                ...
                <td>${renderAdminStatusPill(reward.approval_status || "pending", "reward_approval")}</td>
                <td>
                  <div class="section-actions">
          ...
          .map((order) => {
            ...
            return `
              <tr>
                ...
                <td>${renderAdminStatusPill(payment?.status || order.status || "pending", "payment", status)}</td>
              </tr>
            `;
          })
          .join("")
      ...
      ? data.commissions
          ...
          .map((record) => `
            <tr>
              ...
              <td>${renderAdminStatusPill(record.payout_status || "pending", "payout")}</td>
            </tr>
          `)
          .join("")
      : buildEmptyTableRow(
          ...
          .map((client) => {
            ...
            return `
              <tr>
                ...
                <td>${renderAdminStatusPill(client.status || "active", "account")}</td>
            ...
            .map((order) => {
              ...
              return `
                <tr>
                  ...
                  <td>${renderAdminStatusPill(paymentStatus || "pending", "payment")}</td>
          ...
          .map((clientPackage) => {
            ...
            return `
              <tr>
                ...
                <td>${renderAdminStatusPill(paymentStatus || "pending", "payment")}</td>
                <td>${renderAdminStatusPill(clientPackage.status || "pending", "package")}</td>
              </tr>
            `;
          })
          .join("")
      ...
      ? notifications
          .map((notification) => `
            <tr>
              ...
              <td>${renderAdminStatusPill(notification.category || "system", "notification_category")}</td>
              <td>${escapeHtml(notification.title)}</td>
              <td>${escapeHtml(notification.body)}</td>
              <td>${renderAdminStatusPill(notification.is_read ? "read" : "new", "notification_status")}</td>
            </tr>
          `)
          .join("")
      : buildEmptyTableRow(
            ...
            const actionMarkup =
              record.payout_status === "pending"
                ...
                : record.payout_status === "approved"
                  ...
                  : renderAdminStatusPill(record.payout_status || "pending", "payout");
  ...
  async function loadDashboard(options) {
    ...
    try {
      ...
      notifyAdminShellHydration(data, access);
  ...
  if (newsletterGeneratorFormNode) {
    newsletterGeneratorFormNode.addEventListener("submit", (event) => {
      void handleNewsletterGenerateDraft(event);
    });
  }

  if (newsletterGeneratorPresetsNode) {
    newsletterGeneratorPresetsNode.addEventListener("click", (event) => {
      const button = event.target.closest("[data-newsletter-generator-preset]");
      if (!(button instanceof HTMLButtonElement)) {
        return;
      }
      applyNewsletterGeneratorPreset(button.getAttribute("data-newsletter-generator-preset") || "member_education");
    });
  }

  if (newsletterGeneratorAudienceNode) {
    newsletterGeneratorAudienceNode.addEventListener("change", () => {
      if (newsletterAudienceSegmentNode) {
        newsletterAudienceSegmentNode.value = newsletterGeneratorAudienceNode.value || "active_members";
      }
      dashboardState.newsletterDirty = true;
    });
  }

  if (newsletterGeneratorTypeNode) {
    newsletterGeneratorTypeNode.addEventListener("change", () => {
      if (newsletterTypeNode) {
        newsletterTypeNode.value = newsletterGeneratorTypeNode.value || "update_announcement";
      }
      dashboardState.newsletterDirty = true;
    });
  }
  ...
  if (newsletterOpenManualButtonNode) {
    newsletterOpenManualButtonNode.addEventListener("click", () => {
      setNewsletterManualEditorVisible(true);
    });
  }

  if (newsletterCloseManualButtonNode) {
    newsletterCloseManualButtonNode.addEventListener("click", () => {
      setNewsletterManualEditorVisible(false);
    });
  }
  ...
  if (coachReviewPaginationNode) {
    coachReviewPaginationNode.addEventListener("click", (event) => {
      const button = event.target.closest("[data-pagination-target='coach-review']");
      if (!(button instanceof HTMLButtonElement) || button.disabled) {
        return;
      }

      const nextPage = Number(button.getAttribute("data-page") || 1);
      void loadCoachReviewQueuePage(nextPage, {
        showStatus: false,
      });
    });
  }
  ...
  ensureFinanceReportDefaults();
  bindFinanceWorkspaceNavigation();
  bindNewsletterWorkspaceNavigation();
  setNewsletterManualEditorVisible(false);
  applyNewsletterCapabilities(dashboardState.newsletterCapabilities);
  syncNewsletterGeneratorPresetButtons("member_education");
  applyNewsletterGeneratorPreset("member_education");
  renderCompPlanReference();
