...
const { loadFitbitConnection, saveFitbitConnection, syncLatestFitbitData } = require("./_lib/fitbit");
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const connection = await loadFitbitConnection(supabase, clientId);
    if (!connection?.tokens?.accessToken) {
      throw createHttpError(400, "Fitbit is not connected for this account yet.");
    }

    const result = await syncLatestFitbitData(supabase, clientId, connection, {
      limit: Number(body.limit || 7),
    });

    await saveFitbitConnection(supabase, clientId, {
      ...(result.fitbitConnection || connection),
    ...
    return json(Number(error?.statusCode || 500), {
      error: error?.message || "Unable to sync Fitbit data.",
    });
  }
};
