...
const { loadStravaConnection, saveStravaConnection, syncLatestStravaData } = require("./_lib/strava");
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const connection = await loadStravaConnection(supabase, clientId);
    if (!connection?.tokens?.accessToken) {
      throw createHttpError(400, "Strava is not connected for this account yet.");
    }

    const result = await syncLatestStravaData(supabase, clientId, connection, {
      limit: Number(body.limit || 7),
    });

    await saveStravaConnection(supabase, clientId, {
      ...(result.stravaConnection || connection),
    ...
    return json(Number(error?.statusCode || 500), {
      error: error?.message || "Unable to sync Strava data.",
    });
  }
};
