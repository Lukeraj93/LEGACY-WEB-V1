...
const BUCKET = "client-meal-photos";
const MAX_FILE_SIZE_BYTES = 10 * 1024 * 1024;
const ALLOWED_CONTENT_TYPES = new Set(["image/jpeg", "image/png", "image/webp"]);
const ALLOWED_VIEW_TAGS = new Set(["meal", "secondary", "detail", "packaging"]);
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!assets.length) {
      throw createHttpError(400, "At least one meal photo must be prepared for upload.");
    }
    ...
    for (let index = 0; index < assets.length; index += 1) {
      ...
      const viewTag = normalizeText(asset.viewTag || asset.view_tag || "meal").toLowerCase();
      ...
      if (!ALLOWED_VIEW_TAGS.has(viewTag)) {
        throw createHttpError(400, `Unsupported meal photo view tag: ${viewTag || "unknown"}.`);
      }
      if (!ALLOWED_CONTENT_TYPES.has(contentType)) {
        throw createHttpError(400, `Unsupported meal photo content type: ${contentType || "unknown"}.`);
      }
      if (!Number.isFinite(fileSize) || fileSize <= 0) {
        throw createHttpError(400, "Each meal photo must include a valid file size.");
      }
      if (fileSize > MAX_FILE_SIZE_BYTES) {
        throw createHttpError(400, "Each meal photo must be 10 MB or smaller.");
      }
      ...
      if (error || !data?.token) {
        throw error || createHttpError(500, "Unable to prepare the meal photo upload right now.");
      }
    ...
    return json(200, {
      ...
      bucket: BUCKET,
      assets: signedAssets,
      message: "Meal photo uploads prepared.",
    });
  } catch (error) {
    return json(Number(error?.statusCode || 500), {
      error: error?.message || "Unable to prepare the meal photo upload right now.",
    });
  }
};
