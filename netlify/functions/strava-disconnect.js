...
const { deleteStravaConnection, loadStravaConnection, revokeStravaAccess } = require("./_lib/strava");
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const connection = await loadStravaConnection(supabase, clientId).catch(() => null);

    if (connection?.tokens?.accessToken) {
      await revokeStravaAccess(connection).catch(() => null);
    }

    await deleteStravaConnection(supabase, clientId);
    ...
    return json(Number(error?.statusCode || 500), {
      error: error?.message || "Unable to disconnect Strava.",
    });
  }
};
