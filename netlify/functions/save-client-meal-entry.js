...
async function syncMealDiaryToNutritionLog(supabase, authProfile, options) {
  ...
  const reviewStatus =
    authProfile.role === "client"
      ...
      : "approved";
  const hasManualNutritionLog = Boolean(existing?.id);
  const nextStatus =
    ...
    .upsert(
      {
        ...
        log_date: logDate,
        status: hasManualNutritionLog ? existing?.status || nextStatus : nextStatus,
        balanced_meals_count: hasManualNutritionLog
          ? Number(existing?.balanced_meals_count ?? totals.balancedMeals)
          : totals.balancedMeals,
        ...
        protein_target_hit:
          ...
          || (!hasManualNutritionLog && Number(nutritionPlan.protein_target_g || 0) > 0 && totals.protein >= Number(nutritionPlan.protein_target_g || 0) * 0.95),
        calories_logged: hasManualNutritionLog ? existing?.calories_logged : Math.round(totals.calories),
        protein_logged_g: hasManualNutritionLog ? existing?.protein_logged_g : Math.round(totals.protein),
        carbs_logged_g: hasManualNutritionLog ? existing?.carbs_logged_g : Math.round(totals.carbs),
        fat_logged_g: hasManualNutritionLog ? existing?.fat_logged_g : Math.round(totals.fat),
        ...
        metadata: {
          ...
          mealDiarySync: {
            ...
            preservedManualLog: hasManualNutritionLog,
          },
        },
      },
      {
