...
const { buildFitbitAuthUrl, createOAuthState, isFitbitConfigured, sanitizeReturnPath } = require("./_lib/fitbit");
...
exports.handler = async (event) => {
  ...
  if (!isFitbitConfigured()) {
    return json(400, { error: "Fitbit credentials are not configured yet." });
  }
  ...
  try {
    ...
    return json(200, {
      ok: true,
      authUrl: buildFitbitAuthUrl(state),
      returnPath,
      clientId,
    });
  } catch (error) {
    return json(Number(error?.statusCode || 500), {
      error: error?.message || "Unable to start Fitbit authentication.",
    });
  }
};
