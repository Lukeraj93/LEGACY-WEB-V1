(function initClientDashboard() {
  ...
  const schedulePaginationNode = document.getElementById("client-schedule-pagination");
  ...
  function buildClientLeaderboardSkeleton(rowCount = 3) {
    return Array.from({ length: rowCount }, () => `
      <article class="leaderboard-row leaderboard-row--client-wide">
        ...
        <div class="leaderboard-row__metrics leaderboard-row__metrics--client-wide">
          ...
          <div class="leaderboard-metric leaderboard-metric--pr">
            <span>${buildSkeletonLine("56px", "label")}</span>
            <div class="leaderboard-pr-stack leaderboard-pr-stack--skeleton">
              ${buildSkeletonLine("72px", "metric")}
              ${buildSkeletonLine("72px", "metric")}
              ${buildSkeletonLine("72px", "metric")}
            </div>
          </div>
          ...
          <div class="leaderboard-metric">
            <span>${buildSkeletonLine("42px", "label")}</span>
            <strong>${buildSkeletonLine("58px", "metric")}</strong>
          </div>
          <div class="leaderboard-metric">
            <span>${buildSkeletonLine("32px", "label")}</span>
            <strong>${buildSkeletonLine("44px", "metric")}</strong>
          </div>
          <div class="leaderboard-metric">
            <span>${buildSkeletonLine("46px", "label")}</span>
            <strong>${buildSkeletonLine("44px", "metric")}</strong>
  ...
  function resolveClientLeaderboardVisualTier(entry) {
    const xpPoints = Math.max(0, Number(entry?.xpPoints || entry?.xp_points || 0));
    if (xpPoints >= 900) {
      return "legacy";
    }
    if (xpPoints >= 600) {
  ...
  function resolveClientLeaderboardRankStyle(rank) {
    const numericRank = Math.max(1, Number(rank || 0));
    if (numericRank === 1) {
      return "champion";
    }
    if (numericRank === 2) {
  ...
  function resolveClientLeaderboardSigilLabel(entry) {
    const totalXp = Math.max(0, Number(entry?.xpPoints || entry?.xp_points || 0));
    const level = Math.max(1, Math.min(100, Math.floor(Math.sqrt(totalXp))));
    return `LV${level}`;
  }
  ...
  const SHELL_TEXT_PLACEHOLDER_PATTERNS = [
    /^live client account$/iu,
    /^live crm member identity$/iu,
    /^goal still being defined(?: with coach support\.)?$/iu,
    /^goal: not set$/iu,
    /^goal not set$/iu,
    /^needs onboarding\b/iu,
    /^member id pending$/iu,
    /^portrait selection pending$/iu,
    /^built from live coaching data$/iu,
    /^no paid package balance yet$/iu,
    /^profile syncing$/iu,
    /^package: none yet$/iu,
    /^no roster yet$/iu,
  ];
  ...
  function shouldHideShellText(value, options = {}) {
    const text = normalizeShellText(value);
    if (!text) {
      return true;
    }

    if (/\bdefined\b/iu.test(text)) {
      return true;
    }

    const maxLength = Number.isFinite(options.maxLength) ? Number(options.maxLength) : 50;
    if (!options.allowLong && text.length > maxLength) {
      return true;
    }

    return SHELL_TEXT_PLACEHOLDER_PATTERNS.some((pattern) => pattern.test(text));
  }

  function getRenderableShellText(value, options = {}) {
    const text = normalizeShellText(value);
    return shouldHideShellText(text, options) ? "" : text;
  }

  function setConditionalText(node, value, options = {}) {
    const text = getRenderableShellText(value, options);
    if (node) {
  ...
  async function fetchPaginatedDashboardResource(resource, page, pageSize) {
    ...
    const payload = await response.json().catch(() => ({}));
    if (!response.ok || !payload?.data) {
      throw new Error(payload?.error || "Unable to load the paginated schedule right now.");
    }
    return payload.data;
  }
  ...
  function formatWearableProviderName(providerId) {
    const normalized = String(providerId || "").trim().replace(/_/gu, " ");
    if (!normalized) {
  ...
  function buildWearableIdentityMarkup(primaryName, secondaryLabel) {
    ...
    return [
      name ? `<span class="wearable-identity-name">${escapeHtml(name)}</span>` : "",
      secondary ? `<span class="wearable-identity-handle">${escapeHtml(secondary)}</span>` : "",
    ].filter(Boolean).join("");
  }
              ...
              <article class="wearable-provider-card" data-provider="${escapeHtml(provider.id)}">
                <div class="wearable-provider-card__head">
                  ...
                  <div class="chips wearable-chip-row">
                    ...
                    ${connection?.reward_enabled ? '<span class="chip chip--accent">Active XP source</span>' : ""}
                  </div>
                </div>
  ...
  function renderWearableSummary(workspace) {
    ...
    const rewardSourceLabel = rewardSource ? formatWearableProviderName(rewardSource.provider) : "None selected";
    const latestSyncSummary = latestMetric
      ...
      : connectedCount
        ? "Connected and ready, waiting for the first usable sync."
        : "Sleep, movement, and readiness data will appear after the first pull.";
    const rewardSummary = recentRewards.length
      ? recentRewards.length === 1
        ? "1 wearable-linked XP and coin event is already in your rewards history."
        : `${recentRewards.length} wearable-linked XP and coin events are already in your rewards history.`
      : "No wearable reward events have been created yet.";

    wearableSummaryGridNode.innerHTML = `
      <article class="wearable-summary-item">
        <span>Reward Source</span>
        <strong>${escapeHtml(rewardSourceLabel)}</strong>
        <p>${escapeHtml(rewardSource ? `${rewardSourceLabel} is currently driving wearable XP.` : "You can keep one provider as the active XP source.")}</p>
      </article>
      <article class="wearable-summary-item">
        <span>Latest Daily Sync</span>
        <strong>${escapeHtml(latestMetric?.metric_date ? formatDate(latestMetric.metric_date) : "Not yet")}</strong>
        <p>${escapeHtml(latestSyncSummary)}</p>
      </article>
      <article class="wearable-summary-item">
        <span>Wearable Reward Events</span>
        <strong>${escapeHtml(String(recentRewards.length))} created</strong>
        <p>${escapeHtml(rewardSummary)}</p>
      </article>
    `;
  }
  ...
  function renderFitbitCard(workspace) {
    ...
    const fitbitDisplayName = String(fitbit.profile?.displayName || fitbitConnection?.metadata?.displayName || "").trim();
    const fitbitFullName = String(fitbit.profile?.fullName || fitbitConnection?.metadata?.fullName || "").trim();
    const fitbitUserId = String(fitbit.profile?.userId || fitbitConnection?.external_user_id || "").trim();
    const fitbitIdentityMarkup = buildWearableIdentityMarkup(
      fitbitDisplayName || fitbitFullName,
      fitbitDisplayName && fitbitFullName && fitbitDisplayName !== fitbitFullName ? fitbitFullName : ""
    );
    const hasRecentFitbitData = Boolean(
      latestFitbitMetric
      && (
        latestFitbitMetric.sleep_seconds != null
        || latestFitbitMetric.steps != null
        || latestFitbitMetric.active_minutes != null
      )
    );
    ...
    if (fitbitStatusChipsNode) {
      fitbitStatusChipsNode.innerHTML = [
        ...
        fitbitConnection?.reward_enabled ? '<span class="chip chip--accent">Active XP source</span>' : "",
      ].filter(Boolean).join("");
    }
      ...
      fitbitStatusCopyNode.textContent = configured
        ? connected
          ? hasRecentFitbitData
            ...
            : "Fitbit is connected, but there is no recent Fitbit sleep or movement data available to show yet. Sync again after your next device update when you want it reflected here."
    ...
    if (fitbitAccountNode) {
      if (fitbitIdentityMarkup) {
        fitbitAccountNode.innerHTML = fitbitIdentityMarkup;
      } else {
        fitbitAccountNode.textContent = fitbitUserId || "Not connected";
      }
    }
    ...
    if (fitbitLatestSleepNode) {
      fitbitLatestSleepNode.textContent = latestFitbitMetric?.sleep_seconds != null
        ? formatSleepHoursFromSeconds(latestFitbitMetric.sleep_seconds)
        : connected
          ? "No recent data"
          : "--";
    }
  ...
  function renderStravaCard(workspace) {
    ...
    const athleteFullName = String(strava.profile?.fullName || stravaConnection?.metadata?.fullName || "").trim();
    const athleteUsername = String(strava.profile?.username || stravaConnection?.metadata?.username || "").trim();
    const athleteUserId = String(strava.profile?.userId || stravaConnection?.external_user_id || "").trim();
    const hasRecentActivity = Boolean(
      latestStravaMetric
      && (
        latestStravaMetric.distance_meters != null
        || latestStravaMetric.active_minutes != null
        || latestStravaMetric.steps != null
      )
    );
    ...
    if (stravaStatusChipsNode) {
      stravaStatusChipsNode.innerHTML = [
        `<span class="chip ${configured ? "chip--accent" : ""}">${escapeHtml(configured ? "Configured" : "Needs env keys")}</span>`,
        `<span class="chip">${escapeHtml(connected ? "Connected" : "Not connected")}</span>`,
        stravaConnection?.reward_enabled ? '<span class="chip chip--accent">Active XP source</span>' : "",
      ].filter(Boolean).join("");
    }
    ...
    if (stravaStatusCopyNode) {
      stravaStatusCopyNode.textContent = configured
        ? connected
          ? hasRecentActivity
            ? "Strava is live on this account. Use Sync Now any time you want the latest running and endurance data reflected in your profile."
            : "Strava is connected, but there are no recent Strava activities available to show yet. Complete an activity and sync again when you want it reflected here."
    ...
    if (stravaAccountNode) {
      if (athleteFullName || athleteUsername) {
        const handle = athleteUsername ? `@${athleteUsername.replace(/^@+/, "")}` : "";
        stravaAccountNode.innerHTML = [
          athleteFullName ? `<span class="wearable-identity-name">${escapeHtml(athleteFullName)}</span>` : "",
          handle ? `<span class="wearable-identity-handle">${escapeHtml(handle)}</span>` : "",
        ].filter(Boolean).join("");
      } else {
        stravaAccountNode.textContent = athleteUserId || "Not connected";
      }
    }
    ...
    if (stravaLatestDistanceNode) {
      stravaLatestDistanceNode.textContent = latestStravaMetric?.distance_meters != null
        ? formatDistanceKilometers(latestStravaMetric.distance_meters)
        : connected
          ? "No recent activity"
          : "--";
    }
    ...
    if (stravaLatestActiveNode) {
      stravaLatestActiveNode.textContent = latestStravaMetric?.active_minutes != null
        ? formatMinutesLabel(latestStravaMetric.active_minutes)
        : connected
          ? "No recent activity"
          : "--";
    }
  ...
  async function syncFitbitWorkspace() {
    ...
    try {
      ...
      const refreshedDays = Number(payload.metricsUpserted || 0);
      const rewardsCreated = Number(payload.rewardsCreated || 0);
      const syncMessage = refreshedDays > 0
        ? `Fitbit sync complete. ${refreshedDays} day${refreshedDays === 1 ? "" : "s"} refreshed and ${rewardsCreated} wearable reward event${rewardsCreated === 1 ? "" : "s"} created.`
        : "Fitbit sync completed, but there was no recent Fitbit sleep or movement data available to import in the current sync window.";
      setWearableFeedback(syncMessage, "success");
    } catch (error) {
  ...
  async function startStravaConnectFlow() {
    if (stravaConnectButton?.disabled) {
      setWearableFeedback("Strava is not available on this production build yet.", "error");
      return;
    }
  ...
  async function syncStravaWorkspace() {
    if (stravaSyncButton?.disabled) {
      setWearableFeedback("Strava sync is not available on this production build yet.", "error");
      return;
    }
    ...
    try {
      ...
      const rewardsCreated = Number(payload.rewardsCreated || 0);
      const syncMessage = refreshedDays > 0
        ? `Strava sync complete. ${refreshedDays} day${refreshedDays === 1 ? "" : "s"} refreshed and ${rewardsCreated} wearable reward event${rewardsCreated === 1 ? "" : "s"} created.`
        : "Strava sync completed, but there were no recent Strava activities available to import in the current sync window.";
      setWearableFeedback(syncMessage, "success");
    } catch (error) {
  ...
  async function disconnectStravaWorkspace() {
    if (stravaDisconnectButton?.disabled) {
      setWearableFeedback("Strava is not available on this production build yet.", "error");
      return;
    }
  ...
  function renderClientProfilePage(data) {
    ...
    setConditionalText(profilePassportNoteNode, overview.memberId ? `MEMBER ID ${overview.memberId}` : "Live CRM member identity");
    ...
    setConditionalText(profileGoalMainNode, overview.primaryGoal || "Needs onboarding and a live primary goal from the coaching CRM.");
    ...
    setConditionalText(
      profilePortraitTaglineNode,
      hasPortrait
        ? (
            getRenderableShellText(overview.primaryGoal)
            || getRenderableShellText(
              overview.characterCollectionLabel ? `${overview.characterCollectionLabel} archive` : ""
            )
          )
        : "Portrait selection pending"
    );
    setConditionalText(
      profilePortraitNoteNode,
      hasPortrait
        ? `${overview.completedSessions.length} sessions • ${overview.currentXp} XP • ${overview.currentCoins} coins`
        : ""
    );
    if (profileCoinsNode) {
    ...
    if (profileSummaryMetricsNode) {
      profileSummaryMetricsNode.innerHTML = [
        ...
        .map((item) => {
          const noteText = getRenderableShellText(item.note);
          return `
            <article class="profile-metric-card${noteText ? "" : " profile-metric-card--compact"}">
              <span>${escapeHtml(item.label)}</span>
      ...
      profilePackageSummaryNode.innerHTML = activePackage
        ? `
            <div class="profile-package-summary__grid">
              ...
              <article class="profile-metric-card${getRenderableShellText(activePackage.expires_at ? `Expires ${formatShortDate(activePackage.expires_at)}` : "No expiry set") ? "" : " profile-metric-card--compact"}">
                <span>Bookable now</span>
                <strong>${escapeHtml(String(overview.bookableRemaining))}</strong>
                ${(() => {
                  const noteText = getRenderableShellText(activePackage.expires_at ? `Expires ${formatShortDate(activePackage.expires_at)}` : "No expiry set");
                  return noteText ? `<p>${escapeHtml(noteText)}</p>` : "";
                })()}
              </article>
              <article class="profile-metric-card">
        ...
        : `
            <div class="profile-package-summary__grid">
              <article class="profile-metric-card profile-metric-card--compact">
  ...
  function renderHomeFocus(data) {
    ...
    homeFocusPanelNode.innerHTML = `
      <div class="client-home-focus-grid">
        <article class="client-home-focus-card client-home-focus-card--primary" data-crm-tone="neutral">
          <div class="client-home-focus-card__copy">
            <span class="chip chip--tone-warning">${escapeHtml(action.eyebrow)}</span>
            <h3>${escapeHtml(action.title)}</h3>
            <p>${escapeHtml(action.copy)}</p>
            <div class="section-actions section-actions--compact client-home-focus-actions">
          ...
          <aside class="client-home-focus-progress" aria-label="Current level progress">
            ...
            <div class="client-home-focus-progress-copy">
              <span class="client-home-focus-progress-kicker">Current path</span>
              <strong>${escapeHtml(levelSummary.remainingXp > 0 ? `${formatMetric(levelSummary.remainingXp)} XP to Level ${levelSummary.nextLevel}` : `Level ${levelSummary.currentLevel} complete`)}</strong>
              <p>${escapeHtml(levelSummary.remainingXp > 0 ? `${formatMetric(currentCoins)} coins banked and ready to stack` : "Keep logging sessions to keep the streak alive.")}</p>
            </div>
          </aside>
        </article>
        <div class="client-home-focus-metrics" aria-label="Momentum summary">
          <article>
            <span>This week</span>
            <strong>${escapeHtml(`${formatMetric(weeklyCompletedSessions)} session${weeklyCompletedSessions === 1 ? "" : "s"}`)}</strong>
            <p>${escapeHtml(`${formatMetric(homeSummary.weeklyXp || 0)} XP and ${formatMetric(homeSummary.weeklyCoins || 0)} coins banked this week`)}</p>
          </article>
          <article>
            <span>Bookable now</span>
  ...
  function buildDashboardPaginationMarkup(state, target) {
    ...
    return `
      ...
      <div class="dashboard-table-pagination__controls">
        ...
        >Prev</button>
        ${Array.from(new Set(pageNumbers))
          .map((pageNumber) => `
            <button
  ...
  function renderSchedulePagination(state) {
    if (!schedulePaginationNode) {
      return;
    }

    const markup = buildDashboardPaginationMarkup(state, "scheduleTimeline");
    schedulePaginationNode.hidden = !markup;
    schedulePaginationNode.innerHTML = markup;
  }

  async function loadScheduleTimelinePage(page) {
    if (DASHBOARD_PAGE_KEY !== "home") {
      return;
    }

    const queueState = dashboardState.paginatedTables.scheduleTimeline;
    ...
    renderSchedulePagination(queueState);

    try {
      const payload = await fetchPaginatedDashboardResource("schedule_timeline", queueState.page, queueState.pageSize);
      ...
      console.warn("[LEGACY] Client schedule pagination failed.", error);
      queueState.initialized = true;
    } finally {
      queueState.loading = false;
      renderDashboard(dashboardState.data || buildEmptyDashboardData());
    }
  }

  function ensureHomeSchedulePagination() {
    if (DASHBOARD_PAGE_KEY !== "home") {
      return;
    }

    if (!dashboardState.paginatedTables.scheduleTimeline.initialized) {
      void loadScheduleTimelinePage(dashboardState.paginatedTables.scheduleTimeline.page);
    }
  }
  ...
  function renderSchedule(data) {
    ...
    if (Array.isArray(queueState.rows)) {
      if (!queueState.rows.length) {
        ...
        scheduleRowsNode.innerHTML = queueState.rows
          .map(
            (item) => `
              <tr>
                <td>${escapeHtml(formatDate(item.dateLabel))}</td>
                <td>${escapeHtml(
                  String(item.timeLabel || "").includes("|")
                    ? `${formatTime(String(item.timeLabel || "").split("|")[0])} - ${formatTime(String(item.timeLabel || "").split("|")[1])}`
                    : formatTime(item.timeLabel)
                )}</td>
                <td>${escapeHtml(item.typeLabel)}</td>
                <td>${escapeHtml(item.coachLabel)}</td>
                <td>${buildDashboardStatusMarkup(item.status)}</td>
              </tr>
            `
          )
          .join("");
      ...
      .map(
        (item) => `
          <tr>
            ...
            <td>${escapeHtml(item.typeLabel)}</td>
            <td>${escapeHtml(item.coachLabel)}</td>
            <td class="dashboard-table__cell--status">${buildDashboardStatusMarkup(item.status || item.statusLabel)}</td>
          </tr>
        `
      )
      .join("");

    queueState.totalCount = items.length;
    queueState.totalPages = Math.max(1, Math.ceil(items.length / queueState.pageSize));
    renderSchedulePagination(queueState);
  }
  ...
  function formatLeaderboardWeight(value) {
    const numeric = Number(value || 0);
    if (!Number.isFinite(numeric) || numeric <= 0) {
      return "--";
    }
    return `${new Intl.NumberFormat("en-MY", { maximumFractionDigits: 0 }).format(numeric)} kg`;
  }

  function formatLeaderboardDuration(secondsValue) {
  ...
  function buildLeaderboardPrMetricMarkup(label, prValues) {
    return `
      <div class="leaderboard-metric leaderboard-metric--pr">
        <span>${escapeHtml(label)}</span>
        <div class="leaderboard-pr-stack">
          <div class="leaderboard-pr-chip">
            <small>3RM</small>
            <strong>${escapeHtml(formatLeaderboardWeight(prValues.rm3))}</strong>
          </div>
          <div class="leaderboard-pr-chip">
            <small>5RM</small>
            <strong>${escapeHtml(formatLeaderboardWeight(prValues.rm5))}</strong>
          </div>
          <div class="leaderboard-pr-chip">
            <small>10RM</small>
            <strong>${escapeHtml(formatLeaderboardWeight(prValues.rm10))}</strong>
          </div>
        </div>
      </div>
    `;
  ...
  function renderClientLeaderboard(data) {
    ...
    clientLeaderboardRowsNode.innerHTML = rows
      .map((entry) => {
        ...
        const visualTier = resolveClientLeaderboardVisualTier(entry);
        const rankStyle = resolveClientLeaderboardRankStyle(entry.rank);
        const sigilLabel = resolveClientLeaderboardSigilLabel(entry);
        const goalNote = getRenderableShellText(String(entry.primaryGoal || "").trim() || "Goal still being defined with coach support.");
        const identityNote = getRenderableShellText(String(entry.memberId || "").trim()
          ? `Member ID ${entry.memberId}`
          : "Live client account");

        return `
          <article class="leaderboard-row leaderboard-row--client-wide">
            <div class="leaderboard-row__rank" data-rank-style="${escapeHtml(rankStyle)}">#${escapeHtml(String(entry.rank || ""))}</div>
            <div class="leaderboard-row__avatar" aria-hidden="true">
              <div
                class="leaderboard-row__avatar-shell leaderboard-row__avatar-shell--collectible${avatarUrl ? "" : " leaderboard-row__avatar-shell--fallback"}"
                data-level-tier="${escapeHtml(visualTier)}"
                data-rank-style="${escapeHtml(rankStyle)}"
              >
                <span class="leaderboard-row__avatar-backdrop"></span>
                ${avatarUrl
                  ? `<img alt="" src="${escapeHtml(avatarUrl)}" loading="lazy" decoding="async" />`
                  : buildAvatarFallbackMarkup(displayName, { variant: "compact" })}
                <span class="leaderboard-row__avatar-tone"></span>
                <span class="leaderboard-row__avatar-sigil">${escapeHtml(sigilLabel)}</span>
              </div>
            </div>
            <div class="leaderboard-row__body${identityNote || goalNote ? "" : " leaderboard-row__body--compact"}">
              <div class="leaderboard-row__head${identityNote ? "" : " leaderboard-row__head--compact"}">
                <strong>${escapeHtml(displayName)}</strong>
                ${identityNote ? `<span>${escapeHtml(identityNote)}</span>` : ""}
              </div>
            ...
            <div class="leaderboard-row__metrics leaderboard-row__metrics--client-wide">
              <div class="leaderboard-metric">
                <span>Sessions</span>
                <strong>${escapeHtml(formatMetric(entry.sessionsLogged || 0))}</strong>
              </div>
              <div class="leaderboard-metric">
                <span>Tonnage</span>
                <strong>${escapeHtml(formatLeaderboardWeight(entry.totalWeightLiftedKg || 0))}</strong>
              </div>
              ${buildLeaderboardPrMetricMarkup("Squat PR", {
                rm3: entry.squat3RmKg,
                rm5: entry.squat5RmKg,
                rm10: entry.squat10RmKg,
              })}
              ${buildLeaderboardPrMetricMarkup("Bench PR", {
                rm3: entry.bench3RmKg,
                rm5: entry.bench5RmKg,
                rm10: entry.bench10RmKg,
              })}
              ${buildLeaderboardPrMetricMarkup("Deadlift PR", {
                rm3: entry.deadlift3RmKg,
                rm5: entry.deadlift5RmKg,
                rm10: entry.deadlift10RmKg,
              })}
              <div class="leaderboard-metric">
                <span>5K Time</span>
                <strong>${escapeHtml(formatLeaderboardDuration(entry.fiveKmSeconds))}</strong>
              </div>
              <div class="leaderboard-metric">
                <span>XP</span>
  ...
  if (schedulePaginationNode) {
    schedulePaginationNode.addEventListener("click", (event) => {
      const button = event.target.closest("[data-pagination-target='scheduleTimeline']");
      if (!(button instanceof HTMLButtonElement) || button.disabled) {
        return;
      }

      const nextPage = Number(button.getAttribute("data-page") || 1);
      void loadScheduleTimelinePage(nextPage);
    });
  }
