...
async function main() {
  ...
  const clientMemberId =
    readArg("client-member-id", String(process.env.QA_CLIENT_MEMBER_ID || "").trim())
    || `LC-QA-${Date.now().toString().slice(-6)}-${crypto.randomBytes(2).toString("hex").toUpperCase()}`;
  ...
  runSql(`
    ...
    set member_id = '${escapeSql(clientMemberId)}',
