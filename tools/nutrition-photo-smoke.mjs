...
async function main() {
  ...
  try {
        ...
        assets: [
          {
            ...
            fileSize: mealPhotoFixtureBuffer.length,
          },
        ],
      },
    });
    ...
    const clientPlanner = await api("/.netlify/functions/load-planner-data?refresh=1", {
      token: clientToken,
    });
    ...
    const coachPlanner = await api("/.netlify/functions/load-planner-data?refresh=1", {
      token: coachToken,
    });
