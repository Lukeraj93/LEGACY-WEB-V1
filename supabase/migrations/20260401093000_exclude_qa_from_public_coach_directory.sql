...
as $$
  ...
  where p.role = 'coach'
    and p.status = 'active'
    and coalesce((u.raw_app_meta_data ->> 'qaAccount')::boolean, false) = false
    and coalesce(lower(u.email), '') not like 'qa.%@legacycoaching.com.my'
    and coalesce(lower(p.display_name), '') not like 'legacy qa%'
